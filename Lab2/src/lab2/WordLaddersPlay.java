...
public class WordLaddersPlay {
	private LinkedList<String> wordList;
	private HashMap<String, LinkedList<String>> map;

	public WordLaddersPlay(String filename) {
	...
	public void createAdjacentList() {
		for (int i = 0; i < wordList.size(); i++) {
			...
			LinkedList<String> tempWordList = wordList;
			for (int j = 0; j < tempWordList.size(); j++) {
				if (i != j) {
					char[] c = tempWordList.getFirst().toCharArray();
						...
						if (charList.contains(third)) {
							...
							if (charList.contains(fourth)) {
								...
								if (charList.contains(fifth)) {
									map.get(tempWordList.getFirst()).addLast(
											wordList.get(j));
									tempWordList.removeFirst();
								}
							}
						}
					}
	...
	public static void main(String[] args) {
		WordLaddersPlay w = new WordLaddersPlay("words-5757.dat");
